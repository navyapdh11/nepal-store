 ...
 */
export function findNodeDFS(nodes: TreeNode[], targetId: string): TreeNode | null {
 ...
 */
export function flattenTreeDFS(nodes: TreeNode[], result: TreeNode[] = []): TreeNode[] {
  for (const node of nodes) {
    result.push(node);
    if (node.children) {
...
export const NEPAL_REGIONS: TreeNode[] = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          { id: 'bkt-mun', name: 'Bhaktapur Municipality' }
        ]
      }
    ]
  },
  {
    ...
    name: 'Koshi Province',
    children: [
      { id: 'p1-mor', name: 'Morang District' }
    ]
  }
];
